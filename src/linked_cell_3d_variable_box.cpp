...
// Function to initialize positions in a cubic lattice with particles at the center of the cells
...
//                 if (i < n) {
//                     particles[i].x = (ix + 0.5) * dx;
//                     particles[i].y = (iy + 0.5) * dy;
//                     particles[i].z = (iz + 0.5) * dz;
//                     i++;
//                 }
//             }
//         }
...
void build_cell_list(int n, double L, Particle* particles, std::vector<std::vector<int>>& cell_list, int num_cells) {
...
void calculate_forces_linked_cell(int n, double L ,Particle* particles, double* fx, double* fy, double* fz, const std::vector<std::vector<int>>& cell_list, int num_cells, int total_cells) {
...
void run_simulation(int n, int steps, double dt, double L, double rc, int output_freq, Particle* particles, int num_cells, int total_cells) {
    ...
    build_cell_list(n, L, particles, cell_list, num_cells);
    calculate_forces_linked_cell(n, L, particles, fx, fy, fz, cell_list, num_cells, total_cells);
...
int main() {
    ...
    //open a file
    std::ofstream file("run/time_vs_box_3d_linked_cell.dat");

    for (int i = 0; i < 10; i++) {
        
        auto start = std::chrono::high_resolution_clock::now();
        run_simulation(N, steps, dt, L, rc, output_freq, particles, num_cells, total_cells);
        auto end = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double> elapsed_seconds = end - start;
        file << L << " " << N << " " <<elapsed_seconds.count() << std::endl;
        L += 2.5;
        N = density * pow(L, 3);
        delete[] particles;
        ...
        num_cells = static_cast<int>(L / cell_size);
