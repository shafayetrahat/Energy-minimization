...
void initialize_velocities(int n, Particle* particles) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        particles[i].vx = 0.8 * (rand() / (RAND_MAX + 1.0) - 0.5);
        particles[i].vy = 0.8 * (rand() / (RAND_MAX + 1.0) - 0.5);
...
void calculate_forces_linked_cell(int n, double L, double rc, Particle* particles, double* fx, double* fy, const std::vector<std::vector<int>>& cell_list, int num_cells) {
...
void run_simulation(int n, int steps, double dt, double L, double rc, int output_freq, int seed, Particle* particles, char lattice_type, int num_cells, int total_cells) {
    srand(seed);
    ...
    calculate_forces_linked_cell(n, L, rc, particles, fx, fy, cell_list, num_cells);
...
int main() {
    ...
    //open a file
    std::ofstream file("run/time_vs_box_2d_linked_cell.dat");
    ...
    for (int i = 0; i < 15; i++) {
        ...
        run_simulation(N, steps, dt, L, rc, output_freq, 12345, particles, lattice_type, num_cells, total_cells);
        ...
        num_cells = static_cast<int>(L / cell_size); // number of cells in each dimension
